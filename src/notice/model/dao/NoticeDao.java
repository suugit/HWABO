...
public class NoticeDao {
	...
	public ArrayList<Notice> selectList(Connection conn){
		...
		try {
			...
			while(rset.next()) {
				Notice notice = new Notice( rset.getInt("noticeno"), rset.getString("noticetitle"), 
														   rset.getDate("noticedate"), rset.getString("noticewriter"), 
														   rset.getString("noticecontent"), rset.getString("original_filepath"), 
														   rset.getString("rename_filepath"));
				list.add(notice);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Notice selectOne(Connection conn, int noticeNo) {
		...
		try {
			...
			if(rset.next()) {
			notice = new Notice( rset.getInt("noticeno"), rset.getString("noticetitle"), 
					   rset.getDate("noticedate"), rset.getString("noticewriter"), 
					   rset.getString("noticecontent"), rset.getString("original_filepath"), 
					   rset.getString("rename_filepath"));
			}
	...
	public ArrayList<Notice> selectNewTop3(Connection conn) {
		...
		Statement stmt = null;
		ResultSet rset = null;
		String query = "SELECT * FROM (SELECT ROWNUM RNUM, NOTICENO, NOTICETITLE, NOTICEDATE FROM (SELECT * FROM NOTICE ORDER BY NOTICEDATE DESC)) WHERE RNUM >= 1 AND RNUM <= 3";
